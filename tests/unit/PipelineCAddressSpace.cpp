...
bool lift(void *Opaque, const void *Model, const rp_binary_view *Binary,
...
BOOST_AUTO_TEST_CASE(CreateManagerAndSetLifter) {
  const std::string PipelineOption =
      std::string("--pipeline-path=") + REVNG_ADDRESS_SPACE_PIPELINE;
  ...
  // The entry point becomes a known function, but deliberately without a
  // prototype: `detect-abi` only fills in the ones that have none, so seeding
  // the binary's default here would stop it detecting anything.
  BOOST_CHECK(Model->Functions().contains(
      MetaAddress::fromString(State.Start)));
  BOOST_CHECK(Model->Functions()
                  .at(MetaAddress::fromString(State.Start))
                  .Prototype()
                  .isEmpty());
  BOOST_CHECK(not Model->DefaultPrototype().isEmpty());
  ...
  const rp_llvm_module_callbacks TransformCallbacks{&TransformCalled,
                                                    transformLLVM};
  // NULL means every object the container holds, which for a whole-binary
  // container is the single root object.
  ...
  const auto &LazyModel = Manager->model().get();
  {
  ...
  Manager = rp_manager_create_from_file_address_space(
      ...
      &Error);
  BOOST_REQUIRE(Manager != nullptr);
  const auto &FileModel = Manager->model().get();
