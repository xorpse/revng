...
public:
  llvm::Error lift(const model::Binary &Binary,
    ...
    // Must match `InstructionTranslator`: BasicBlockID, instruction size,
    // isJT, entry of the owning function, then the live locals.
    ...
    for (MetaAddress Seed : Seeds) {
      ...
      for (unsigned InstructionCount = 0; InstructionCount != 64;
        ...
        // Code belongs to a function only once it has been outlined, so the
        // owner is invalid here, exactly as in `root`.
        Builder.CreateCall(NewPC,
                           { BasicBlockID(Address).toValue(&Output),
                             ...
                             MetaAddress::invalid().toValue(&Output) });
