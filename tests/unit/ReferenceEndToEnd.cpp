...
BOOST_AUTO_TEST_CASE(IsolateThroughEmitC) {
  ...
  const rp_container_identifier *Identifier =
      rp_manager_get_container_identifier_from_name(Manager.get(), "decompiled."
                                                                   "tar.gz");
  const rp_kind *Kind =
      rp_manager_get_kind_from_name(Manager.get(), "decompiled");
  ...
  std::unique_ptr<rp_buffer, decltype(&rp_buffer_destroy)> DirectArtifact(
      rp_manager_produce_artifact(Manager.get(), "emit-c-as-single-file",
                                  "decompiled.c", "decompiled-to-c", 0, nullptr,
                                  Error.get()),
      rp_buffer_destroy);
  ...
  BOOST_REQUIRE(rp_manager_transform_mlir_module(
      Manager.get(), "emit-c", "functions.mlir", &TransformCallbacks,
      Error.get()));
