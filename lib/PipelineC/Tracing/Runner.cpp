...
// rp_initialise causes an abort, this is done since there can only be one call
...
// have the first command be an rp_initialise for ease of use.
static void handleRpInitialize(RunnerContext &Context,
                               ArgumentsRef Arguments,
                               ReturnRef Return) {
  softAssert(Context, false, "rp_initialise invoked mid-trace");
}
...
public:
  CommandHandler() {
    Registry.insert({ "rp_initialise", handleRpInitialize });
...
llvm::Error Trace::run(const revng::tracing::RunTraceOptions Options) const {
  ...
  // Allow rp_initialise as first command and rp_shutdown as last, in all other
  // cases the trace is malformed and needs to be aborted
  const size_t FirstCommandI = FirstCommand.Name == "rp_initialise" ? 1 : 0;
