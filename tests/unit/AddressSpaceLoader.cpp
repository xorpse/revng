...
// The model requires a segment to start at a generic address, while a caller
// naturally describes a mapping by the address its code runs at.
BOOST_AUTO_TEST_CASE(SegmentStartsAreGeneric) {
  TestAddressSpace AddressSpace;
  auto Loaded = revng::loader::loadAddressSpace(AddressSpace);
  BOOST_REQUIRE(bool(Loaded));

  for (const model::Segment &Segment : Loaded->Model->Segments())
    BOOST_CHECK(Segment.StartAddress().isGeneric());

  BOOST_CHECK(Loaded->Model->verify(false));
}
