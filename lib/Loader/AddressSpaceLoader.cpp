...
loadAddressSpace(const AbstractAddressSpace &AddressSpace,
  ...
  for (const Mapping &Mapping : AddressSpace.mappings()) {
    ...
    // A segment is just a span of memory, so the model requires its start to
    // be a generic address. Callers naturally describe a mapping by the address
    // code will run at, so convert rather than reject: a code address and its
    // generic form denote the same location.
    model::Segment Segment({ Mapping.Start.toGeneric() });
    Segment.VirtualSize() = Mapping.VirtualSize;
    ...
    // Every binary importer gives its segments a struct type covering the
    // whole span, and the decompiler dereferences it without checking the
    // moment emitted code refers to segment data -- a jump table, say. A
    // host-described address space has no sections or symbols to place in it,
    // so the struct stays empty, exactly as the Mach-O importer leaves it.
    if (Mapping.VirtualSize > 0) {
      auto &&[Struct, Type] = Result.Model
                                ->makeStructDefinition(Mapping.VirtualSize);
      Struct.CanContainCode() = Mapping.Executable;
