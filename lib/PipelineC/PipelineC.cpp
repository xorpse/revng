...
static bool _rp_is_initialised() {
  return Initialized;
}

static bool _rp_initialise(int argc, const char *argv[],
...
static rp_manager *_rp_manager_create_from_address_space(
  ...
  if (materialise_for_serialisation) {
...
static rp_manager *_rp_manager_create_from_file_address_space(
  ...
  return _rp_manager_create_from_address_space(
      &Callbacks, nullptr, materialise_for_serialisation, pipeline_flags_count,
      pipeline_flags, execution_directory, error);
}
...
static bool _rp_manager_materialise_address_space(rp_manager *manager,
                                                  rp_error *error) {
  revng_check(manager != nullptr);
...
static char *
_rp_invalidations_serialise(const rp_invalidations *invalidations) {
  revng_check(invalidations != nullptr);
  std::string Out;
...
// For example: rp_initialise will call
// `wrap<"rp_intialize">(rp_initialise, ...)`
// which in turn will call `_rp_initialise`
