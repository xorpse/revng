#pragma once
#ifndef __APPLE__
#include_next "llvm/Support/YAMLTraits.h"
#else
...
public:
  DefaultInserter(IO &TheIO, T &Sequence) :
    TheIO(TheIO), Sequence(Sequence) {}

  auto &preflightElement(unsigned Index) {
    return SequenceTraits<T>::element(TheIO, Sequence, Index);
  }
...
template<typename T>
DefaultInserter<T> getInserter(...) {
  return DefaultInserter<T>{};
}

template <typename T, typename Context>
std::enable_if_t<has_SequenceTraits<T>::value, void>
yamlize(IO &io, T &Seq, bool, Context &Ctx) {
  decltype(getInserter<T>(std::declval<T>())) Inserter(io, Seq);
  if ( has_FlowTraits< SequenceTraits<T>>::value ) {
    ...
    for(unsigned i=0; i < count; ++i) {
      ...
      if ( io.preflightFlowElement(i, SaveInfo) ) {
        yamlize(io, Inserter.preflightElement(i), true, Ctx);
        Inserter.postflightElement(i);
