...
[`fugue-decompiler-revng`](https://github.com/fugue-re/fugue-decompiler-revng) implements the same path in Rust, with a backend that
...
The raw Rust declarations in [`revng-sys`](https://github.com/xorpse/revng-rs) mirror every type API and input
...
not be retained or disposed. The consumer's own `inkwell` wrapper hides the raw
